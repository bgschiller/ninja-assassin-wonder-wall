...
def add_preds(g, x, queue=None, preds=[]):
    """ONLY call this on directed acyclic graphs. On a graph with a cycle, it is an infinite recusion.
    This function depth-first traverses the graph, keeping track of all predecessors and directing the appropriate edges.
    For example if we call the function on a graph:
        a --> b --> c
              |
              -> d --> e
    will add edges a -> c, a -> d, a -> e, and b -> e (the direct edges are already present)
...
def make_assumption(pair, pairs_count, given_graph, relationships, reverse_checked=False, spaces=2):
    """Given an edge to start with, make_assumption fixes that edge in the given orientation.
    Then it fixes all the edges implied by this orientation. If this produces a DAG, recurse on another edge. Otherwise, try the opposite orientation.
    pair -- a 2-tuple (x,y) indicating that we should start by assuming x -> y
    pairs_count -- an instance of Counter, representing the number of times each 2-tuple is represented in the relationship table, minus the number of times it is represented in the graph.
    given_graph -- the edges that are already directed. make_assumption will honor these directions.
    relationships -- a list of 3-tuples (x,y,z) where y is x's WW and z is x's NA
    reverse_checked -- if true, we have already concluded that the opposite orientation leads to a cyclic graph, so if this orientation doesn't work out, there is no solution.
    spaces -- should not be set at top level call. spaces determines the indentation level, so that the output visually represents the recursion depth."""
    ...
    while len(to_process) > 0:
        ...
        for (x,y,z) in relationships: #make all the implications we can from that edge
            ...
            if to_add is not None and (to_add not in working_graph.edges_iter()):
                print '{spaces}{a} -> {b} is given, so relationship {xyz} => {w} -> {u}'.format(spaces=' '*spaces, a=a, b=b, xyz=(x,y,z), w=to_add[0], u=to_add[1])
                working_graph.add_edge(*to_add)
                to_process.append(to_add)
        if nx.is_directed_acyclic_graph(working_graph):
            # If we have an edge a->b, and an edge b->c, we need to add the edge a->c so that we can make assumptions based on that.
    ...
    if nx.is_directed_acyclic_graph(working_graph):
        ...
        return make_assumption(next_pair, working_count, working_graph, relationships, spaces=spaces+2)
        ...
        return make_assumption(list(reversed(pair)), pairs_count, given_graph, relationships, reverse_checked=True, spaces=spaces)
    else:
        print '{spaces}Cycle in graph, and we\'ve already checked the other side! Returning false.'.format(spaces=' '*spaces)
...
def find_ordering(relationships):
    """main function. Given a table of relationships, either print a solution or print 'no solution' and list the simple cycles.
    ...
    solvable, order_graph = make_assumption(starting_pair, pair_count, order_graph, relationships)
    if solvable:
